"""
Generate seed SQL from the Python seed data.
Run with: python -c "from app.seed.export_sql import export; export()"
Or: python app/seed/export_sql.py
"""
import json
import sys
sys.path.insert(0, "..")
...
def generate_sql():
    ...
    for svc in all_services:
        ...
        lines.append(f"""
INSERT INTO emergency_services (name, service_type, trauma_grade, location, address, phone, has_icu, has_ventilator, has_blood_bank, has_trauma_team, capacity, verified)
VALUES (
    '{svc["name"].replace("'", "''")}',
    '{svc["type"]}',
    {grade},
    ST_GeomFromText('SRID=4326;POINT({svc["lng"]} {svc["lat"]})', 4326),
    ...
    for proto in PROTOCOL_DATA:
        ...
        text = proto["text"].replace("'", "''")
        tags = "{" + ",".join(f'"{t}"' for t in proto["tags"]) + "}"

        lines.append(f"""
INSERT INTO protocol_chunks (source, language, protocol_type, chunk_text, tags)
VALUES ('{source}', '{lang}', '{ptype}', '{text}', '{tags}');""")
...
if __name__ == "__main__":
    ...
    with open("infra/seed.sql", "w") as f:
        f.write(sql)
    print(f"Generated infra/seed.sql ({len(sql)} bytes)")
    print(f"Services: {len(TN_HOSPITALS) + len(TN_AMBULANCES) + len(TN_POLICE) + len(TN_BLOOD_BANKS)}")
    print(f"Protocols: {len(PROTOCOL_DATA)}")